...
import { Terminal, Play, Pause, Trash2, Circle, X } from "lucide-react";
import {
  ACTION_COLORS,
  ACTION_LABELS,
  ENTITY_LABELS,
...
export default function GlobalConsole({ userRole }: GlobalConsoleProps) {
  const [logs, setLogs] = useState<LogEntry[]>([]);
  const [isConnected, setIsConnected] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [isMinimized, setIsMinimized] = useState(true); // Empieza minimizado
  const [isVisible, setIsVisible] = useState(true);
  const consoleRef = useRef<HTMLDivElement>(null);
  const eventSourceRef = useRef<EventSource | null>(null);

  // Solo mostrar para ADMINISTRADOR - usar variable en lugar de return temprano
  const isAdmin = userRole === "ADMINISTRADOR";

  useEffect(() => {
    if (!isAdmin || isPaused || !isVisible) return;
    ...
    return (
      <div className="fixed bottom-4 right-4 z-50 print:hidden">
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 bg-zinc-900 text-white rounded-lg shadow-lg hover:bg-zinc-800 transition-colors"
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-[500px] bg-zinc-900 rounded-lg shadow-2xl border border-zinc-700 overflow-hidden print:hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2 bg-zinc-800 border-b border-zinc-700">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-1.5 hover:bg-zinc-700 rounded text-zinc-400 hover:text-white transition-colors ml-1"
            title="Minimizar"
          >
